 ...
 *
 */
public class JPlogPDescriptor extends AbstractMolecularDescriptor implements
        IMolecularDescriptor
{
    ...
    public DescriptorValue calculate(IAtomContainer container)
    {
        ...
        try {
            ...
            AtomContainerManipulator.
                    percieveAtomTypesAndConfigureUnsetProperties(struct);
            CDKHydrogenAdder hAdder = CDKHydrogenAdder.getInstance(struct.
                    getBuilder());
        ...
        return new DescriptorValue(getSpecification(), getParameterNames(),
                getParameters(), new DoubleResult(logP),
                getDescriptorNames());
    }
        ...
        if (bondCount > 0) {
            ...
            if (neighbour.getSymbol().equals("C")) {
                if (carbonylConjugated(neighbour)) {
                    ...
                    double formalOxState = getNumMoreElectronegativethanCarbon(
                            neighbour);
    ...
    protected int getDefaultSpecial(IAtom atom)
    {
        ...
        if (atom.isAromatic()) {
            ...
            toadd = singleBondPolar + doubleBondPolar + tripleBondPolar
                    + aromaticBondPolar;
        }
        return toadd;
    }
    ...
    protected int getCarbonSpecial(IAtom atom)
    {
        ...
        switch (numConnections) {
            ...
            default:
                toadd = 0; // C???
                if (singleBondPolar > 0 || doubleBondPolar > 0
                        || aromaticBondPolar > 0 || tripleBondPolar > 0) {
                    toadd = 1; // C??Polar
                }
        }
        return toadd;
    ...
    protected boolean checkAlphaCarbonyl(IAtom atom, String symbol)
    {
        for (IBond bond : atom.bonds()) {
            ...
            for (IBond bond2 : next.bonds()) {
                ...
                if (next2.getSymbol().equals(symbol) && bond2.getOrder().
                        numeric() == 1) {
                    return true;
                }
            }
    ...
    protected boolean electronWithdrawing(IAtom atom)
    {
        ...
        if (symbol.equals("N") || symbol.equals("O") || symbol.equals("S")
                || symbol.equals("F") || symbol.equals("Cl")
    ...
    {
        if (atom.getSymbol().matches("O") || atom.getSymbol().matches("S")
    ...
    protected boolean doubleBondHetero(IAtom atom)
    {
        for (IBond bond : atom.bonds()) {
            ...
            if (!(bond.isAromatic()) && isPolar(neighbour) && bond.getOrder().
                    numeric() == 2) {
                return true;
            }
        }
    ...
    protected boolean carbonylConjugated(IAtom atom)
    {
        for (IBond bond : atom.bonds()) {
            IAtom next = bond.getOther(atom);
            if (!(bond.isAromatic()) && bond.getOrder().numeric() == 1
                    && doubleBondHetero(next)) {
                return true;
            }
        }
        return false;
